export default function (eleventyConfig) {
  ...
  const CHORD_RE_GLOBAL = /\b([A-G][#b]?)(m|min|maj|dim|aug|sus[24]?|add)?(\d+)?(\/[A-G][#b]?)?\b/g;
  function isChordLine(line) {
    const stripped = line.trim();
    if (!stripped) return false;
    const withoutChords = stripped.replace(CHORD_RE_GLOBAL, '').replace(/[\s|,.\-–—:x\d()\/\[\]]/g, '');
  ...
  eleventyConfig.addFilter("songDifficulty", (content) => {
    ...
    for (const c of chords) {
      if (!BEGINNER_CHORDS.has(c) && !INTERMEDIATE_CHORDS.has(c)) hasHard = true;
